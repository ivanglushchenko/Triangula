 ...
 */
case class BoardHash(val edges: List[Int], val triangles: List[Int]) extends Ordered[BoardHash] {
  ...
  def compare(that: BoardHash): Int = compareLists(edges, that.edges) match {
    case 0 => compareLists(triangles, that.triangles)
    case n => n
  }

  def compareLists(l1: List[Int], l2: List[Int]): Int = (l1, l2) match {
    case (Nil, _ :: _) => -1
    case (Nil, _)      => 0
    case (hd1 :: tl1, hd2 :: tl2) =>
      if (hd1 < hd2) -1
      else if (hd1 > hd2) 1
      else compareLists(tl1, tl2)
  }

  /**
   * Checks if one list of indices is smaller than the other.
   */
  def isSmaller(l1: List[Int], l2: List[Int]): Boolean = compareLists(l1, l2) < 0
